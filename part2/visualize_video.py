...
from part2_dataset import CLASS_NAMES, IMAGENET_MEAN, IMAGENET_STD
...
def find_latest_checkpoint(root_dir: str) -> Optional[str]:
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        for priority, file_name in enumerate(("best_model.pth", "best_model_iou.pth", "best.pt", "last.pt", "model.pth")):
...
def main() -> None:
    ...
    if checkpoint_path is None:
        raise FileNotFoundError(
            "No checkpoint found under part2/dogs. "
            "Pass --checkpoint path/to/best_model.pth after training part2."
        )
    ...
    model = MobileNetDetector(pretrained=False).to(device)
