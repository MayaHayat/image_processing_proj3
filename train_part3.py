...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train the Part 3 MobileNetV3 detector.")
    parser.add_argument("--dataset-root", type=str, default=os.path.join(project_root, "dogs-and-people-1"))
    ...
    parser.add_argument("--output-dir", type=str, default=os.path.join(project_root, "outputs", "part3"))
    parser.add_argument("--run-name", type=str, default=None)
    parser.add_argument("--max-train-samples", type=int, default=None)
    parser.add_argument("--max-valid-samples", type=int, default=None)
    parser.add_argument("--max-test-samples", type=int, default=None)
    parser.add_argument("--use-coco", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--coco-annotation-path", type=str, default=os.path.join(project_root, "custom_annotations.json"))
    parser.add_argument("--coco-image-dir", type=str, default=os.path.join(project_root, "custom_coco_dataset"))
    parser.add_argument("--coco-max-samples", type=int, default=None)
    parser.add_argument("--device", type=str, default=get_default_device())
    args = parser.parse_args()
    args.dataset_root = resolve_project_path(args.dataset_root)
    args.output_dir = resolve_project_path(args.output_dir)
    args.coco_annotation_path = resolve_project_path(args.coco_annotation_path)
    args.coco_image_dir = resolve_project_path(args.coco_image_dir)
    return args
...
def resolve_optional_coco_paths(args: argparse.Namespace) -> Tuple[str, str] | Tuple[None, None]:
...
def build_optimizer(model: MobileNetV3FixedSlotDetector, args: argparse.Namespace) -> AdamW:
    return AdamW(
        [
